function [ im, claMap,evaluationIdx ] = KNNclassificationscript( varargin )
%k nearest neighbor
...
mdl =ClassificationKNN.fit(trainNormalised,trLab,'NumNeighbors',1,'distance','euclidean'); 


%
% evaluation on test sample
%
classTest= predict(mdl,testNormalised); 
...
claMap = zeros(r,c);
im = zeros(r,c);
% [ claMap ] = KNNclassificationMap( feature,label,mdl,m,s );
% claMap = claMap.*(~mask);
