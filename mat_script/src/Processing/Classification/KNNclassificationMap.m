function [ claMap ] = KNNclassificationMap( data,label,model,m,s )
...
for i = 1:nb_r
    ...
    for j = 1:nb_c
        if j == nb_c
            [ ~,~,feature,~ ] = sampleData( temp_data(:,(j-1)*nc+1:end,:),temp_label(:,(j-1)*nc+1:end),0 );
            ...
            classAll= predict(model,allNormalised);             
            if i == nb_r
            ...
            [ ~,~,feature,~ ] = sampleData( temp_data(:,(j-1)*nc+1:j*nc,:),temp_label(:,(j-1)*nc+1:j*nc),0 );
            if isa(m,'double')&&isa(s,'double')
                allNormalised = (feature - repmat(m,size(feature,1),1))./repmat(s,size(feature,1),1);% feature scaling for test samples
            else
                allNormalised = feature;
