function [ epdroa ] = EPDROA( f_data,o_data )
%UNTITLED2 Summary of this function goes here
%   Detailed explanation goes here
[~,~,d] = size(f_data);
epdroa = sum(sum(abs(f_data(2:end-1,2:end-1,:)./f_data(3:end,2:end-1,:)),1),2)./sum(sum(abs(o_data(2:end-1,2:end-1,:)./o_data(3:end,2:end-1,:)),1),2);
