...
for cv_bin = 1:length(nbBin)
    ...
    %% Topology matrix
    [ T_ul{cv_bin,cv_ovl}, Wv_ul{cv_bin,cv_ovl} ] = MIMA_MAPPER_EN_D_MS( se1Data,nbBin(cv_bin),ovLap(cv_ovl) );
    [ T_br{cv_bin,cv_ovl}, Wv_br{cv_bin,cv_ovl} ] = MIMA_MAPPER_EN_D_MS( se2Data,nbBin(cv_bin),ovLap(cv_ovl) );
    end
end
...
save('mima_en_d_ms_p','M','oa','nbBin','ovLap','T_ul','T_br','Wv_ul','Wv_br','map1','map2','Mdl_rf','scores','teLab','Me','oae','-v7.3')
