...
%% Similarity matrix
tmp = cat(1,trLab,zeros(length(teLab),1),zeros(nbUnlabel,1),trLab,zeros(length(teLab),1),zeros(nbUnlabel,1));
...
%% Topology matrix
se1Data = cat(1,trainSE1,testSE1,unlabelSE1);
[ T_ul ] = MIMA_MAPPER_fd_A( se1Data );

se2Data = cat(1,trainSE2,testSE2,unlabelSE2);
...
testFeat = feat(size(trainSE1,1)+1:size(trainSE1,1)+size(testSE1,1),:);
